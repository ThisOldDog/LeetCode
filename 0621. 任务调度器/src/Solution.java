...
public class Solution {
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        LinkedList<int[]> execute = new LinkedList<>();
        for (char c : tasks) {
            task[c - 'A'] += 1;
        }
        int timeConsuming = 0;
        while (total > 0) {
            ...
            for (int i = 0; i < task.length; i++) {
                if (task[i] > 0) {
                    ListIterator<int[]> iterator = execute.listIterator();
                    while (iterator.hasNext()) {
                        int[] v = iterator.next();
                        if (task[i] > v[1]) {
                            iterator.previous();
                            iterator.add(new int[]{i, task[i]});
                            continue task;
                        }
                    }
                    if (execute.size() < n + 1) {
            ...
            List<int[]> doTask = execute.size() > n + 1 ? execute.subList(0, n + 1) : execute;
            doTask.forEach(t -> task[t[0]] -= 1);
            total -= doTask.size();
            timeConsuming += total > 0 ? Math.max(n + 1, doTask.size()) : doTask.size();
            execute.clear();
        }
